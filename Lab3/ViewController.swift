...
//  Lab3
//
//  Created by Krunal Shah on 2022-03-20.
//
...
//            print("In datatask: \(data)")
            ...
            if let weather = self.parseJson(data: data) {
                ...
                DispatchQueue.main.async {
                    ...
                    self.tempInFLabel.text = "\(weather.current.temp_f)F"
                    self.conditionLabel.text = weather.current.condition.text
                    
                    let config = UIImage.SymbolConfiguration(paletteColors: [.systemYellow, .systemBlue, .systemPurple])
